...
import { formatDateInput, formatEuDate } from "../../helpers/dates";
import { useSelector } from "react-redux";

export const UpdateDateForm = ({date, updateDate}) => {
  const { id } = useParams();
  ...
  const onSubmit = (data) => {
    axios
      .patch(`http://localhost:3000/api/citas/${date.id}`, {
        id_paciente: id,
      ...
      .then((response) => {
        ...
        if (response.data.id = date.id) {
          setShowError(null);
          setShowSuccess(
            `Se ha guardado la cita correctamente con el ID #${response.data.id}`);
        ...
        >
          {servicesList &&
            servicesList.map((service) => (
              <option key={service.id} value={service.id} selected={service.id == date.id_servicio ? true : false}>
        ...
        >
          {employeesList &&
            employeesList.map((employee) => (
              <option key={employee.id} value={employee.id} selected={employee.id == date.id_empleado ? true : false}>
      ...
      <div className="flex flex-col">
        ...
        <div className="grid gap-2 lg:grid-cols-2 md:grid-cols-2 grid-cols-1">
          <div className="w-full">
            <input
              defaultValue={formatDateInput(date.fecha)}
              type="date"
              id="date"
              className="mt-3 mb-1 peer relative h-10 w-full rounded border border-slate-200 px-4 text-sm text-slate-500 placeholder-transparent outline-none transition-all autofill:bg-white invalid:border-pink-500 invalid:text-pink-500 focus:border-cyan-500 focus:outline-none invalid:focus:border-pink-500 focus-visible:outline-none disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400"
              {...register("date", {
