...
export const UpdateServiceForm = ({ setServices, service }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = (data) => {
    axios
      .patch(`http://localhost:3000/api/servicios/${service.id}`, {
      ...
      .then((response) => {
        if (
          ...
          setShowError(null);
          setShowSuccess(`Se ha actualizado el servicio correctamente`);
          setServices((prev) =>
            prev.map((serv) => (serv.id === response.data.id ? response.data : serv))
          );
        } else {
          setShowSuccess(null);
